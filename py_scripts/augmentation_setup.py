...
class AugmentationParameters():
    ...
    methods = ['random_deletion', 'synonym_replacement', 'shuffle_within_segments', 'label_wise_token_replacement', 'mention_replacement'] # ['back_translation']
...
for data_size in aug_params.data_size_range:
    ...
    if("back_translation" in aug_params.methods):
        ...
        with Pool() as pool:
            pool.map(augment_data, [(X_train, Y_train, data_size, p, num_new_docs, aug_method) for p in aug_params.p_range for num_new_docs in aug_params.num_new_docs_range for aug_method in aug_params.methods])
